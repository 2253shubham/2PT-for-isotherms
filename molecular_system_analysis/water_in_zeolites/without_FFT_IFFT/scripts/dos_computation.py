...
def mod_prinaxes(MI):
    ...
    for i in range(0, len(MI[0]), 3):
        for j in range(1, len(MI)):
            ...
            if int(np.abs(c[0] - c[1])) == 1:
                MI[j, i : i + 3, int(c[2])] = np.cross(
                    MI[j, i : i + 3, min(int(c[0]), int(c[1]))],
                    MI[j, i : i + 3, max(int(c[0]), int(c[1]))],
                )
            else:
                MI[j, i : i + 3, int(c[2])] = np.cross(
                    MI[j, i : i + 3, max(int(c[0]), int(c[1]))],
    ...
    for i in range(len(ctr)):
        if np.linalg.det(MI[0, 3 * i : 3 * i + 3, :]) < 0:
            MI[0, 3 * i : 3 * i + 3, int(ctr[i][2])] = -MI[
                0, 3 * i : 3 * i + 3, int(ctr[i][2])
            ]
    return MI
...
def dos_data2(x, y, dt):
    dos_pr, ff2 = density_of_states2(
        total_autocorr(corr_3D(x, y)), dt
    )  # calculating density of states with data in principal axes frame
...
if __name__ == "__main__":
    ...
    p1 = u1.trajectory[0 : len(u1.trajectory) : 2]
    print(p1)
    p2 = u2.trajectory[0 : len(u2.trajectory) : 2]
    print(p2)
    N, md_MI, vel, time, pos, mass, dt, tot_mass = extract_data_from_universe(
    ...
    print(tot_dos_val)
    tr_dos, tr_dos_val, ff = dos_data(
        com_vel, m, dt
    )  # Computing translational density of states
    print(tr_dos_val)
    rot_dos, rot_dos_val, ff = dos_data(
        w_npt, mit, dt
    )  # Computing rotational density of states
    print(rot_dos_val)
    outd1 = open("tot_dos_for_5fs_dt_for_" + runnum + ".txt", "w")
    outd2 = open("tr_dos_for_5fs_dt_for_" + runnum + ".txt", "w")
    outd3 = open("rot_dos_for_5fs_dt_for_" + runnum + ".txt", "w")
    outd1.truncate(0)
    outd2.truncate(0)
    outd3.truncate(0)
    ...
    for i in range(len(mvac_axes[0, :])):
        print(
            np.float(time[i] * 1e12),
            np.float(mvac_axes[0, :][i] * (Na / 1e3)),
            file=outd1,
        )
    outd1.close()
