...
def trans_iag_helmholtz(vol, m, T):
    ...
    for i in Nr:
        x = (
            -kb * T * i * ((np.log(vol * ((2 * np.pi * m * kb * T) / (h**2)) ** (1.5))))
        )  # + np.log(*(((np.pi*(T**3))/(13.706211107457026*20.998505222742857*39.467682045442515))**(0.5))*0.5))
        for j in range(i):
            x = x + kb * T * np.log(j + 1)
        A_ig.append(x)
        x = 0
...
if __name__ == "__main__":
    ...
    r = (
        ((((2 * np.pi * tot_mass * kb * T) / (h**2)) ** (1.5)) * (vol / 1))
        * (
            (
                (np.pi * (T**3))
                / (13.706211107457026 * 20.998505222742857 * 39.467682045442515)
    ...
    print(rot_fl)
    gas_rot_dos, solid_rot_dos = ref_rotational_dos(
    ...
    rot_s_cho, rot_s_qho, rot_s_gas, rot_s_cho1, rot_s_qho1 = rot_entropy(
        W_Shs, ff, rot_dos, solid_rot_dos, gas_rot_dos
    )  # rotational part entropy
    rot_e_cho, rot_e_qho, rot_e_gas, rot_e_cho1, rot_e_qho1 = rot_energy(
        W_Ehs, ff, rot_dos, solid_rot_dos, gas_rot_dos
    )  # rotational part internal energy
    rot_h_cho, rot_h_qho, rot_h_gas, rot_h_cho1, rot_h_qho1 = rot_helmholtz(
