...
def extract_velocity_and_time(univ, x):
    ...
    for i in univ:
        ...
        time.append(i.time)
    return np.array(vel, dtype=np.float_)*1e2, np.array(time, dtype=np.float_)*(1e-12) # vel -> (40001x100x3), time -> (40001x1)
...
def extract_pos_com_MI_prinaxes(univ, x, y, red):
    ...
    posa = np.array(pos, dtype=np.float_)*(1e-10) # array to store positions in SI units
    la = np.array(l, dtype=np.float_).reshape(x,y*3,3)*(1e-20)/(1e3*Na) # array to store moment of inertia in SI units
    return posa, la
...
def prinaxes(MI,x):
    ...
    a3 = [] # list to store eigen vectors (column wise) corresponding to each eigen values
    a4 = [] # list to store eigen values for each MIT in descending order
    for i in range(len(a1)):
        a3.append(w[i][:,a1[i]]) # storing eigen vectors corresponding to decreasing eigen values
        a4.append(v[i][a1[i]]) # storing eigen values in descending order
    a3 = np.array(a3).reshape(x,-1,3) # shape -> 40001x75x3
    a4 = np.array(a4)
    diag = []
    for i in a4:
        diag.append(np.diag(i)) # storing Principal Moments of Inertia (PMIT) which is the diagonal matrix of eigen values
    diaga = np.array(diag).reshape(x,-1,3) # shape -> 40001x75x3
    diagainv = np.linalg.inv(diaga.reshape(-1,3,3)).reshape(x,-1,3) # calculating inverse of PMIT
...
def angular_momentum(mass, vec, vel):
    ...
    return L_i.reshape(-1,len(mass),3).sum(1).reshape(len(vel),-1,3) # shape -> 40001x25x3

# calculating angular velocity from angular momentum (MITinv * L)
def rot_angular_velocity(MIinv, L, k):
    w = np.matmul(MIinv.reshape(-1,3,3),L.reshape(-1,3,1)).reshape(k,-1,1) # shape -> 40001x75x1
    return w  

# calculating rotational velocity from angular velocity (w x r)
def rot_from_ang_velocity(w, d, x, y):
    v = np.cross(w.reshape(-1,1,3),d.reshape(-1,x,3)).reshape(y,-1,3) # shape -> 40001x100x3
    return v  
...
def prinaxes_data(rel_dis, vel, np_prinaxes, mass):
    ...
    Lt = ((np.transpose(np_prinaxes.reshape(-1,3,3), (0,2,1)))@(np.transpose(L.reshape(-1,1,3), (0,2,1)))).reshape(len(vel),-1,3) # calculating angular momnetum in principal axes frame
    w_np = rot_angular_velocity(MIinv, Lt.reshape(len(vel),-1), len(vel)) # calculating angular velocity
...
def dos_data(x, y, ff, dt, k):
    ...
    dos_req, tdos = integ_DoS(dos_pr, ff[:len(ff)-1], dt, k) # calculating total density of states    
    return dos_req, tdos
...
if __name__ == '__main__':
    ...
    print(outfile1)
    print(md_energy)
    u1_ref = mda.Universe(analysis_tpr_file,trr_file) # creating universe with velocities
    print(u1_ref)
    u2_ref = mda.Universe(analysis_tpr_file,xtc_file) # creating universe with positions
    print(u2_ref)
    ...
    print(p2)
    N_ref, md_MI_ref, vel_ref, time, pos_ref, mass_ref, ff, dt, tot_mass = extract_data_from_universe(p1, p2, wat_atoms, u2_ref.residues)
    print(N_ref)
    print(vel_ref.shape)
    np_MI_ref, np_MIinv_ref, np_prinaxes_ref = prinaxes(md_MI_ref,len(vel_ref))
    print(np_prinaxes_ref.shape)
    rel_dis_ref, com_vel_ref = box_frame_data(pos_ref, vel_ref, mass_ref, len(u1_ref.residues[0].atoms))
    MIT_ref, w_npt_ref = prinaxes_data(rel_dis_ref, vel_ref, np_prinaxes_ref, mass_ref)
    m_ref = np.repeat(np.tile(mass_ref.transpose(), int(len(vel_ref[0])/len(mass_ref))).transpose(), repeats=3, axis=1)
    ...
    tot_dos_ref, tot_dos_val_ref = dos_data(vel_ref, m_ref, ff, dt, N_ref) # Computing total density of states
    ...
    tr_dos_ref, tr_dos_val_ref = dos_data(com_vel_ref, m_ref, ff, dt, N_ref) # Computing translational density of states
    print(tr_dos_val_ref)
    rot_dos_ref, rot_dos_val_ref = dos_data(w_npt_ref, mit_ref, ff, dt, N_ref) # Computing rotational density of states
...
# Total system data
    ...
    rot_dif_sp_ref = np.float(rot_dos_ref[0]*(T*kb)/(12*tot_mass*1)) # Rotational Self diffusion coefficient (p)
    rot_fl = rot_hs_fluidicity(tot_mass, den, rot_dos_ref[0], T) # Rotational fluidicity
    print(rot_fl)
    gas_rot_dos_ref, solid_rot_dos_ref = ref_rotational_dos(rot_fl, rot_dos_val_ref, rot_dos_ref, ff[0:len(ff)-1]) # gas and solid rotational dos
    W_Shs = hs_rot_entropy_wf(tot_mass, den, rot_fl, rot_dos_ref[0]) # hs approx. rotational entropy weighing function 
    ...
    rot_s = rot_entropy(W_Shs, ff, rot_dos_ref, solid_rot_dos_ref, gas_rot_dos_ref) # rotational part entropy 
    rot_e = rot_energy(W_Ehs, ff, rot_dos_ref, solid_rot_dos_ref, gas_rot_dos_ref) # rotational part internal energy
    rot_h = rot_helmholtz(W_Ahs, ff, rot_dos_ref, solid_rot_dos_ref, gas_rot_dos_ref) # rotational part helmholtz energy
    print(rot_s)
    
    # Translational system data
    reference = md_energy - 1.5*(1-rot_fl)*(beta**-1)
    tr_dif_sp_ref = np.float(tr_dos_ref[0]*(T*kb)/(12*tot_mass*1)) # Translational Self diffusion coefficient (p)
