...
def extract_velocity_and_time(univ, x):
    ...
    print(psutil.virtual_memory())
    return vel*1e2, (time.reshape(-1))*(1e-12) # vel -> (40001x100x3), time -> (40001x1)
...
def Ixx_cal(vec, x, y, m, v):
    ...
    for i in range(len(vec)):
        cal[i] = np.sum((m*(vec[i][:,x]**2 + vec[i][:,y]**2)).reshape(-1,v),axis=1) 
    return cal
...
def dos_data(x, y, ff, dt):
    dos_pr = density_of_states(total_autocorr(corr_3D(x,y))) # calculating density of states with data in principal axes frame
    dos_pr = dos_pr.reshape(len(dos_pr),1)
    dos_req, tdos = integ_DoS(dos_pr, ff[:len(ff)-1], dt) # calculating total density of states    
    return dos_req, tdos
...
def entropy(W_Shs, ff, y, ys, yg):
    ...
    S_cho1 = (integrate.simps(ysm.T,ff[1:(int)((len(y))/2)].T))*kb # Solid Entropy (assuming classical harmonic oscillator)
    S_cho =  S_cho1 + S_gas # Entropy (assuming classical harmonic oscillator)
    return np.float(S_cho)

# internal energy 
def energy(W_Ehs, ff, y, ys, yg):
    E_cho = (integrate.simps(yg[1:(int)((len(y))/2)].T*W_Ehs,ff[1:(int)((len(y))/2)].T) + integrate.simps(ys[1:(int)((len(y))/2)].T,ff[1:(int)((len(y))/2)].T))*(beta**(-1)) # Energy (assuming classical harmonic oscillator)
...
def helmholtz(W_Ahs, ff, y, ys, yg):
    ...
    A_cho = (integrate.simps(ygm.T,ff[1:(int)((len(y))/2)].T) + (integrate.simps(ysm.T,ff[1:(int)((len(y))/2)].T)))*beta**(-1) # Helmholtz free energy (assuming classical harmonic oscillator)
...
if __name__ == '__main__':
    ...
    enerfile = sys.argv[5]
    runnum = sys.argv[6]
    reffile = sys.argv[7] # system argument to specify translational gas reference dos file
    refpropfile = sys.argv[8] # system argument to specify translational reference state properties file
    ...
    print(p2)
    N, md_MI, vel, time, pos, mass, ff, dt, tot_mass = extract_data_from_universe(p1, p2, wat_atoms, u2.residues)
    print(N)
    print(vel.shape)
    ...
    outd.close() 
    print(tot_dos_val) 
    tr_dos, tr_dos_val = dos_data(com_vel, m, ff, dt) # Computing translational density of states
    print(tr_dos_val)
    rot_dos, rot_dos_val = dos_data(w_npt, mit, ff, dt) # Computing rotational density of states
    print(rot_dos_val)
...
# Total system data
    ...
    md_energy = np.float(enerfile)*1e3/Na + 750629*1e3/Na # total md energy*1e3/Na + 750629*1e3/Na
    ...
    print(rot_fl)
    gas_rot_dos, solid_rot_dos = rotational_dos(rot_fl, rot_dos_val, rot_dos, ff[0:len(ff)-1]) # gas and solid rotational dos
    ...
    rot_s = entropy(W_Shs, ff, rot_dos, solid_rot_dos, gas_rot_dos) # rotational part entropy 
    rot_e = energy(W_Ehs, ff, rot_dos, solid_rot_dos, gas_rot_dos) # rotational part internal energy
    rot_h = helmholtz(W_Ahs, ff, rot_dos, solid_rot_dos, gas_rot_dos) # rotational part helmholtz energy
    ...
    tr_h_gas = tr_fl*tr_u_ex + (a + b*(N*tr_fl) + c*((N*tr_fl)**2) + d*((N*tr_fl)**3) + e*((N*tr_fl)**4)) 
    tr_h = tr_h_gas + helmholtz(0, ff, tr_dos, solid_tr_dos, gas_tr_dos) # translational part entropy 
    tr_e = N*tr_fl*tr_e_gas_ref + energy(0, ff, tr_dos, solid_tr_dos, gas_tr_dos) # translational part internal energy
    tr_s = (N*tr_fl*tr_e_gas_ref - tr_h_gas)/T + entropy(0, ff, tr_dos, solid_tr_dos, gas_tr_dos) # translational part helmholtz energy
    ...
    print("Total Helmholtz free energy of the system scales with N (normalized by beta) = ", tot_h/(kb*T), file = out)
    print("Total Internal energy of the system scales with N (normalized by beta) = ", tot_e/(kb*T), file = out)
    ...
    print("ln of Total Partition function of the system (assuming classical harmonic oscillator for solid part of the system) scales with N = ", tot_ln_q, file = out)
    print("\n\n Rotational System Data: \n", file = out)
    ...
    print("Total number of gas Rotational DOS in the system = ", rot_fl*rot_dos_val, file = out)
    print("Total number of solid Rotational DOS in the system = ", (1-rot_fl)*rot_dos_val, file = out)
    print("Total Rotational Helmholtz free energy of the system scales with N (normalized by beta) = ", rot_h/(kb*T), file = out)
    print("Total Rotational energy of the system scales with N (normalized by beta) = ", rot_e/(kb*T), file = out)
    print("Total Rotational Entropy (scaled by kb) = ", rot_s/kb, file = out)
    print("ln of Rotational Partition function of the system (assuming classical harmonic oscillator for solid part of the system) scales with N = ", rot_ln_q, file = out)
    print("\n\n Translational System Data: \n", file = out)
    print("Total Translational Density of states in the system = ", tr_dos_val, file = out)
    print("Translational Fluidicity / Fraction of gas = ", tr_fl, file = out)
    print("Total number of gas Translational DOS in the system = ", tr_fl*tr_dos_val, file = out)
    print("Total number of solid Translational DOS in the system = ", (1-tr_fl)*tr_dos_val, file = out)
    print("Total Translational Helmholtz free energy of the system scales with N (normalized by beta) = ", tr_h/(kb*T), file = out)
    print("Total Translational energy of the system scales with N (normalized by beta) = ", tr_e/(kb*T), file = out)
    print("Total Translational Entropy (scaled by kb) = ", tr_s/kb, file = out)
    print("ln of Translational Partition function of the system (assuming classical harmonic oscillator for solid part of the system) scales with N = ", tr_ln_q, file = out)
    out.close()
