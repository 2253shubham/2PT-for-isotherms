...
W_Acho = [] # weighting function for helmholtz free energy corresponding to solid assuming classical harmonic oscillator
W_Acho.append(0)
W_Aqho = [] # weighting function for helmholtz free energy corresponding to solid assuming quantum harmonic oscillator
W_Aqho.append(0)
W_Scho = [] # weighting function for entropy corresponding to solid assuming classical harmonic oscillator
W_Scho.append(0)
W_Sqho = []  # weighting function for entropy corresponding to solid assuming quantum harmonic oscillator
W_Sqho.append(0)
W_Eqho = [] # weighting function for energy corresponding to solid assuming quantum harmonic oscillator
W_Eqho.append(0)
ysm1 = [] # modified solid DOS parameter used to calculate differnt properties assuming classical harmonic oscillator
ysm1.append(0)
ysm2 = [] # modified solid DOS parameter used to calculate differnt properties assuming quantum harmonic oscillator
ysm2.append(0) 
...
delta = math.sqrt((np.pi*kb*T)/m)*((6/np.pi)**(2/3))*(den**(1/3))*(2*y[0])/(9*N) # normalized diffusivity constant
...
q = 0
def fluidicity(f):
    return (2*(delta**(0-9/2))*(f**(4.5*q+15/2))-6*(delta**(0-3))*(f**(3*q+5))-(delta**(0-3/2))*(f**(2.5*q+7/2))+6*(delta**(0-3/2))*(f**(1.5*q+5/2))+2*(f**(q+1))-2)
...
dif_hsfp = (kb*T*y[0])/(12*m*N*frac) # diffusivity of the gas component assuming gas as a hard sphere (fp)
dif_cep = (3/8)*math.sqrt((kb*T)/(np.pi*m))/(den*dia**2) # hard sphere Chapman Enskog diffusion (p)
...
a1 = ((beta*h*ff[1:len(ff)])/(np.exp(beta*h*ff[1:len(ff)])-1)).reshape(-1,1)
a2 = (np.log(1 - np.exp(0 - beta*h*ff[1:len(ff)]))).reshape(-1,1)
W_Sqho = a1-a2
ysm2 = W_Sqho*ys[1:len(ff)]
S_cho1 = (integrate.simps(ysm1.T,ff[1:len(ff)].T))*kb # Entropy (assuming classical harmonic oscillator)
S_qho1 = (integrate.simps(ysm2.T,ff[1:len(ff)].T))*kb # Entropy (assuming quantum harmonic oscillator)
S_hs = (integrate.simps(yg[1:len(yg)].T*W_Shs,ff[1:len(yg)].T))*kb # Entropy (assuming quantum harmonic oscillator)
S_cho = (S_cho1 + S_hs)
S_qho = (S_qho1 + S_hs)

W_Ahs = 0.5 - (sh/(3*kb)) # weighting function for helmholtz free energy corresponding to hard sphere gas
W_Acho = (np.log(beta*h*ff[1:len(ff)])).reshape(-1,1)
ysm1  = W_Acho*ys[1:len(ff)]
q = (1 - np.exp(0 - beta*h*ff[1:len(ff)]))/np.exp(0 - beta*h*ff[1:len(ff)]).reshape(-1,1)
W_Aqho = np.log(q)
ysm2 = W_Aqho*ys[1:len(ff)]

A_cho1 = refener + (integrate.simps(ysm1.T,ff[1:len(ff)].T))*beta**(-1) # Helmholtz free energy (assuming classical harmonic oscillator)
A_qho1 = refener + (integrate.simps(ysm2.T,ff[1:len(ff)].T))*beta**(-1) # Helmholtz free energy (assuming quantum harmonic oscillator)
A_hs = (integrate.simps(yg[1:len(yg)].T*W_Ahs,ff[1:len(yg)].T))*beta**(-1)
A_cho = (A_cho1 + A_hs)
A_qho = (A_qho1 + A_hs)
Q = 0 - (A_cho)/(kb*T) # partition function of the system (assuming classical harmonic oscillator) 
Q_qho = 0 - (A_qho)/(kb*T) # partition function of the system (assuming quantum harmonic oscillator) 
...
print("Entropy of the system (assuming classical harmonic oscillator for solid part of the system) per molecule basis = ", np.float(S_cho/(N*kb)), file = out)
print("Entropy of the system (assuming quantum harmonic oscillator for solid part of the system) per molecule basis = ", np.float(S_qho/(N*kb)), file = out)
print("Solid Entropy contribution (assuming classical harmonic oscillator) per molecule basis = ", np.float(S_cho1/(N*kb)), file = out)
print("Solid Entropy contribution (assuming quantum harmonic oscillator) per molecule basis = ", np.float(S_qho1/(N*kb)), file = out)
print("Gas Entropy contribution per molecule basis = ", np.float(S_hs/(N*kb)), file = out)
print("Entropy of the system (assuming classical harmonic oscillator for solid part of the system) scales with N = ", np.float(S_cho/(sf*kb)), file = out)
print("Entropy of the system (assuming quantum harmonic oscillator for solid part of the system) scales with N = ", np.float(S_qho/(sf*kb)), file = out)
print("Solid Entropy contribution (assuming classical harmonic oscillator) scales with N = ", np.float(S_cho1/(sf*kb)), file = out)
print("Solid Entropy contribution (assuming quantum harmonic oscillator) scales with N = ", np.float(S_qho1/(sf*kb)), file = out)
print("Entropy hs wf = ", np.float(W_Shs), file = out)
print("Gas Entropy contribution scales with N = ", np.float(S_hs/(sf*kb)), file = out)
print("Helmholtz free energy of the system (assuming classical harmonic oscillator for solid part of the system) per molecule basis = ", np.float(A_cho/(N*T*kb)), file = out)
print("Helmholtz free energy of the system (assuming quantum harmonic oscillator for solid part of the system) per molecule basis = ", np.float(A_qho/(N*T*kb)), file = out)
print("Solid Helmholtz free energy contribution (assuming classical harmonic oscillator) per molecule basis = ", np.float(A_cho1/(N*T*kb)), file = out)
print("Solid Helmholtz free energy contribution (assuming quantum harmonic oscillator) per molecule basis = ", np.float(A_qho1/(N*T*kb)), file = out)
print("Gas Helmholtz free energy contribution per molecule basis = ", np.float(A_hs/(N*T*kb)), file = out)
print("Helmholtz free energy of the system (assuming classical harmonic oscillator for solid part of the system) scales with N = ", np.float(A_cho/(sf*T*kb)), file = out)
print("Helmholtz free energy of the system (assuming quantum harmonic oscillator for solid part of the system) scales with N = ", np.float(A_qho/(sf*T*kb)), file = out)
print("Solid Helmholtz free energy contribution (assuming classical harmonic oscillator) scales with N = ", np.float(A_cho1/(sf*T*kb)), file = out)
print("Solid Helmholtz free energy contribution (assuming quantum harmonic oscillator) scales with N = ", np.float(A_qho1/(sf*T*kb)), file = out)
print("Helmholtz hs wf = ", np.float(W_Ahs), file = out)
print("Gas Helmholtz free energy contribution scales with N = ", np.float(A_hs/(sf*T*kb)), file = out)
print("Total Energy of the system (assuming classical harmonic oscillator for solid part of the system) per molecule basis = ", np.float(A_cho/(N*T*kb)+S_cho/(N*kb)), file = out)
print("Total Energy of the system (assuming quantum harmonic oscillator for solid part of the system) per molecule basis = ", np.float(A_qho/(N*T*kb)+S_qho/(N*kb)), file = out)
print("Solid contribution to Total Energy (assuming classical harmonic oscillator) per molecule basis = ", np.float(A_cho1/(N*T*kb)+S_cho1/(N*kb)), file = out)
print("Solid contribution to Total Energy (assuming quantum harmonic oscillator) per molecule basis = ", np.float(A_qho1/(N*T*kb)+S_qho1/(N*kb)), file = out)
print("Gas contribution to Total Energy per molecule basis = ", np.float(A_hs/(N*T*kb)+S_hs/(N*kb)), file = out)
print("Total Energy of the system (assuming classical harmonic oscillator for solid part of the system) scales with N = ", np.float(A_cho/(sf*T*kb)+S_cho/(sf*kb)), file = out)
print("Total Energy of the system (assuming quantum harmonic oscillator for solid part of the system) scales with N = ", np.float(A_qho/(sf*T*kb)+S_qho/(sf*kb)), file = out)
print("Solid contribution to Total Energy (assuming classical harmonic oscillator) scales with N = ", np.float(A_cho1/(sf*T*kb)+S_cho1/(sf*kb)), file = out)
print("Solid contribution to Total Energy (assuming quantum harmonic oscillator) scales with N = ", np.float(A_qho1/(sf*T*kb)+S_qho1/(sf*kb)), file = out)
print("Gas contribution to Total Energy scales with N = ", np.float(A_hs/(sf*T*kb)+S_hs/(sf*kb)), file = out)
