...
print(enermd)
refener = enermd - ((3*N)*(1 - fl))/(beta) - N*fl*enermdref # reference energy
"""
...
A_gas = N*fl*u_ex + (a + b*(N*fl) + c*((N*fl)**2) + d*((N*fl)**3) + e*((N*fl)**4))
...
b2 = ((beta*h*ff[1:len(ff)])/(np.exp(beta*h*ff[1:len(ff)])-1)).reshape(-1,1)
W_Eqho = b1+b2
ysm2 = W_Eqho*ys[1:len(ff)]
E_cho1 = refener + (integrate.simps(ys[1:len(ys)].T,ff[1:len(ys)].T))*(beta**(-1)) #Total energy (solid part) chm
E_qho1 = refener + (integrate.simps(ysm2.T,ff[1:len(ff)].T))*(beta**(-1)) # Total energy (solid part) qhm
...
print("Helmholtz free energy of the system (assuming classical harmonic oscillator for solid part of the system) per molecule basis = ", np.float(A_cho/(N*kb*T)), file = out)
print("Solid Helmholtz free energy contribution (assuming classical harmonic oscillator) per molecule basis = ", np.float(A_cho1/(N*kb*T)), file = out)
print("Gas Helmholtz free energy contribution per molecule basis = ", np.float(A_gas/(N*kb*T)), file = out)
print("Helmholtz free energy of the system (assuming classical harmonic oscillator for solid part of the system) scales with N = ", np.float(A_cho/(sf*kb*T)), file = out)
print("Solid Helmholtz free energy contribution (assuming classical harmonic oscillator) scales with N = ", np.float(A_cho1/(sf*kb*T)), file = out)
print("Gas Helmholtz free energy contribution scales with N = ", np.float(A_gas/(sf*kb*T)), file = out)
...
print("Gas Entropy contribution per molecule basis = ", np.float(S_gas/(N*kb)), file = out)
...
print("Gas Entropy contribution scales with N = ", np.float(S_gas/(sf*kb)), file = out)
...
print("Total Energy of the system (assuming classical harmonic oscillator for solid part of the system) scales with N = ", np.float(E_cho/(sf*kb*T)), file = out)
print("Solid contribution to Total Energy (assuming classical harmonic oscillator) scales with N = ", np.float(E_cho1/(sf*kb*T)), file = out)
print("Gas contribution to Total Energy scales with N = ", N*fl*enermdref/(sf*kb*T), file = out)
print("Total Energy of the system (assuming classical harmonic oscillator for solid part of the system) per molecule basis = ", np.float(E_cho/(N*kb*T)), file = out)
print("Solid contribution to Total Energy (assuming classical harmonic oscillator) per molecule basis = ", np.float(E_cho1/(N*kb*T)), file = out)
print("Gas contribution to Total Energy per molecule basis = ", fl*enermdref/(kb*T), file = out)
...
print("ln of Partition function of the system (assuming classical harmonic oscillator for solid part of the system) scales with N = ", np.float(Q), file = out)
...
print("System with N = " + numstr + " and run number = " + numrun + " completed!!") # indication that the particular run is completed
