...
popt1, pconv1 = curve_fit(func, N_org[1:], data1[1:], p0 = [800,10]) # p0=[100,100,-8,5,-100]
a1 = popt1[0]
b1 = popt1[1]
...
plt.plot(N_org[1:],func(N_org[1:],*popt1),"r*",label = "curve_fit_single_site_LI_model")
...
plt.plot(LIl/((a1-LIl)*bpc), LIl, label="Langmuir Isotherm fitted by calculated parameters")
...
for i in range(len(N)): 
   print(np.float(N[i]),np.float(tot[i,0]),np.float(tot[i,1]),np.float(tot[i,2]),np.float(tot[i,3]),np.float(tot[i,4]), file=out) 
out.close()
out = open("plot7_manuscript_data_scaled_by_kbT.txt", "w") 
out.truncate(0) 
for i in range(len(N)): 
   print(np.float(N[i]),np.float(tot[i,0]*beta),np.float(tot[i,1]*beta),np.float(tot[i,2]*beta),np.float(tot[i,3]*beta),np.float(tot[i,4]*beta), file=out) 
out.close()
